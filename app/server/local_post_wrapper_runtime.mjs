...
export async function runHmdaoPostRuntime({
  backendName,
  backendLabel,
  runtimePath,
  request,
  ...
  const outputDir = path.dirname(path.resolve(String(request.outputPath || process.cwd())));
  ...
  await Promise.all([
    ...
    fs.rm(resultPath, { force: true }).catch(() => {}),
  ]);
