...
> 生成时间：2026-08-08 08:45:07  |  工具：debug-agent（自动静态分析，无需人工验证）
...
- **入口定义**：extension\scan.js:538 — `function scanPage(networkAssets) {`
- **该入口内部调用的关键符号**：
  - extension\scan.js:73 — `// 根因：此前 scanTab 在 B站播放页仍注入 scanPage(ISOLATED) + extractFreshVideoUrl(MAIN) +`
- **调用该入口的位置**：
  - extension\background.js:15
  - extension\background.js:1528
  - extension\background.js:1618
  - extension\model-api-capture.js:25
  - extension\model-api-capture.js:655
