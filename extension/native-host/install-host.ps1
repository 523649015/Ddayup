...
$manifestObj = Get-Content $manifestSrc -Raw | ConvertFrom-Json
...
# 关键：Chrome 原生主机 manifest 严禁 BOM。PS5.1 的 Set-Content -Encoding UTF8 会写 BOM，
# 故改用 .NET UTF8Encoding(false) 明确写无 BOM 文件，否则 Chrome 解析失败报
# "Access to the specified native messaging host is forbidden"。
$json = $manifestObj | ConvertTo-Json -Compress
[System.IO.File]::WriteAllText($manifestOut, $json, [System.Text.UTF8Encoding]::new($false))
Write-Host "[OK] 已生成主机 manifest（无 BOM）: $manifestOut"
